...
window.onload = function() {
    ...
    for (const item of menu) {
        createItem(item.id, item.name, "../src/" + item.image, item.price);
        order[item.name] = 0;
    }
}
...
function createItem(item_id, item_name, item_image, item_price) {
    ...
    const itemHTML = `
    <button onClick="handleItemClick('${item_id}')" class="relative flex flex-row gap-2 rounded-xl bg-neutral-50 shadow-sm inset-shadow-sm">
...
function handleItemClick(item_id) {
    ...
    quantityElement.textContent = order[item.name];
